...
export class LancamentoAvariaPage {
  ...
  ionViewDidEnter() {
    console.log('ionViewDidEnter LancamentoAvariaPage');
    // setTimeout(() => {
    ...
    this.http.get(this.url + uriBuscaChassi).subscribe(
      (res) => {
        ...
        if (this.responseData.sucesso) {
          this.authService.hideLoading();
          this.openLancarChassisAvaria(this.responseData.retorno, byScanner);
        } else {
          this.authService.hideLoading();
          if (this.modoOperacao == 1 || partChassi.length < 17) {
            ...
            this.openLancarChassisAvaria([partChassi], byScanner);
          } else {
           this.openModalErro(this.responseData.mensagem, byScanner);
          }
        }
